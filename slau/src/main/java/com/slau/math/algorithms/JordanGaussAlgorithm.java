    ...
    public Float[] getSolution() {
        try {
            if (this.checkIsThereZeroLines()) {
                throw new Exception("No solutions (zero lines before unit view)");
            }
            this.bringToUnitMatrix();
            if (this.checkIsThereZeroLines()) {
                throw new Exception("No solutions (zero lines after unit view)");
            }
    ...
    private void bringToUnitMatrix() {
        for (int diagI = 0; diagI < this.lSystem.size; diagI++) {
            ...
            for (int rowBelowDiag = diagI + 1; rowBelowDiag < this.lSystem.size; rowBelowDiag++) {
                ...
                this.solutionsVector.set(rowBelowDiag,
                        rowBelowDiabOldVectorVal - valToSubstractInVector);
